    ...
    <div className="text-xs relative">
      <div className="flex items-center justify-between px-2 pb-1.5 mb-1">
        <div className="flex items-center gap-2">
          ...
          {error && isMock && <span className="text-[10px] text-yellow-500/70">Demo</span>}
        </div>
        <LastUpdated
            ...
            return (
              <div key={i} className="relative grid grid-cols-2 px-2 py-0.5">
                <div
                  ...
                  style={{ width: `${width}%` }}
                />
                <span className="font-mono text-loss relative z-10">{formatPrice(price)}</span>
                <span className="font-mono text-right relative z-10">{qty.toFixed(4)}</span>
              </div>
            );
          })}
      </div>
          ...
          return (
            <div key={i} className="relative grid grid-cols-2 px-2 py-0.5">
              <div
                ...
                style={{ width: `${width}%` }}
              />
              <span className="font-mono text-gain relative z-10">{formatPrice(price)}</span>
              <span className="font-mono text-right relative z-10">{qty.toFixed(4)}</span>
            </div>
          );
        })}
